...
const AddAudit = () => {
  ...
  const [allChecklists, setAllChecklists] = useState([]); // Danh sách checklists gốc
  const [filteredChecklists, setFilteredChecklists] = useState([]); // Danh sách checklists đã lọc
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-6">
          ...
          <select
            ...
            className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
            required
            disabled={filteredChecklists.length === 0} // Vô hiệu hóa nếu không có checklist nào
          >
            <option value="">
